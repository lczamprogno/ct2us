...
g_kernel = g_kernel[None, None, :, :].detach().clone().to(device=device, dtype=torch.float32)
...
class UltrasoundRendering(torch.nn.Module):
    ...
    def map_dict_to_array(self, dictionary, arr):
        ...
        if not isinstance(arr, torch.Tensor):
            arr_tensor = torch.as_tensor(arr, dtype=torch.long, device=device)
        else:
            arr_tensor = arr.to(device=device)
        ...
        # Get unique keys
        keys = torch.unique(arr_tensor).to(device=device)
        ...
        try:
            ...
            mapping = torch.zeros(max_key + 1).to(device=device)
            mapping[keys] = values
